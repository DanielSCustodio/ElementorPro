...
define( 'SEARCH_IN_PLACE_VERSION', '1.0.79');
...
if(!function_exists('search_in_place_troubleshoot'))
{
	...
	if(!function_exists("CodePeopleSearchInPlace")){
		function CodePeopleSearchInPlace(){
			...
			wp_enqueue_style('codepeople-search-in-place-style', plugin_dir_url(__FILE__).'css/codepeople_shearch_in_place.css', array(), SEARCH_IN_PLACE_VERSION);
			wp_enqueue_script('codepeople-search-in-place', plugin_dir_url(__FILE__).'js/codepeople_shearch_in_place.js', array('jquery'), SEARCH_IN_PLACE_VERSION);
